...
struct GameHook : public D3D11Hook, public IOHookHigh, public MainGameController {
		...
		if (info.shader) {
			...
			for (const auto & c : info.cbuffers)
				if (bound_cbuffers.count(c.name)) {
					...
					D3D11Hook::CSSetConstantBuffers(c.bind_point, 1, &new_b);
				}
			// Set the Inputs
	...
	virtual bool requestGameReset() override {
		return resetGame();
	}
	...
	virtual bool readTarget(const std::string & name, int W, int H, int C, half * data) {
		auto t = targets_.find(name);
		if (t != targets_.end())
	...
	virtual bool readTarget(const std::string & name, int W, int H, int C, float * data) {
		auto t = targets_.find(name);
		if (t != targets_.end())
			return t->second->read(W, H, C, data);
		LOG(WARN) << "Target '" << name << "' not found!";
	...
	virtual bool readTarget(const std::string & name, int W, int H, int C, uint8_t * data) {
		auto t = targets_.find(name);
		if (t != targets_.end())
			return t->second->read(W, H, C, data);
		LOG(WARN) << "Target '" << name << "' not found!";
	...
	virtual bool readTarget(const std::string & name, int W, int H, int C, uint16_t * data) {
		auto t = targets_.find(name);
		if (t != targets_.end())
			return t->second->read(W, H, C, data);
		LOG(WARN) << "Target '" << name << "' not found!";
	...
	virtual bool readTarget(const std::string & name, int W, int H, int C, uint32_t * data) {
		auto t = targets_.find(name);
		if (t != targets_.end())
			return t->second->read(W, H, C, data);
		LOG(WARN) << "Target '" << name << "' not found!";
