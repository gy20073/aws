...
const double DEFAULT_CAPTURE_FPS = 10;
...
struct Capture: public GameController {
	bool is_init = false, capturing = false;
	...
	void init() {
		if (!is_init) {
			...
			if (GetEnvironmentVariableA("CAPTURE_FPS", tmp, sizeof(tmp)))
				capture_fps = atof(tmp);
	...
	virtual RecordingType recordFrame(uint32_t frame_id) {
		if (capturing) {
			init();
			capture = last_capture + 1. / capture_fps < time();
			if (capture) {
				last_capture = time();
				return DRAW;
			}
	...
	virtual void endFrame(uint32_t frame_id) {
		...
		if (gs.size() > 5 && capturing) {
			gs[gs.size() - 1] = ',';
			gs = gs + "\"timestamp\": " + std::to_string(time()) + ",\"key_state\": [";
			for (auto &k : key_state) {
				if (k.second)
					gs += std::to_string((int)k.first) + ",";
			}
			gs[gs.size() - 1] = ']';
			gs = gs + "}";
