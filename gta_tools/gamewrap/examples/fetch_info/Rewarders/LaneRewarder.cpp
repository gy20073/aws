...
float LaneRewarder::computeReward(Vehicle vehicle) {
	...
	Vector3 laneCenter, currentPosition;
	float d, a, direction, reward;
	int nodeID;
	tNode node;
	tLink link;
	cannotaccessroadinfo = true;
	currentPosition = ENTITY::GET_ENTITY_COORDS(vehicle, FALSE);
	isOnroad = PATHFIND::IS_POINT_ON_ROAD(currentPosition.x, currentPosition.y, currentPosition.z, 0);
	if (!isOnroad) return -1.0;
...
void LaneRewarder::populateNodes(const char* pathsfile) {
	...
	for (object; object; object = object->NextSiblingElement()) {
		if (object->Attribute("class", "vehiclenode")) {
			...
			node.attr.water = false;
			node.attr.speed = -1;
			node.attr.special = -1;
			node.attr.density = -1;
			node.attr.highway = false;
			node.attr.noGPS = false;
			node.attr.tunnel = false;
			node.attr.cantGoLeft = false;
			node.attr.leftTurnsOnly = false;
			node.attr.offRoad = false;
			node.attr.cantGoRight = false;
			node.attr.noBigVehicles = false;
			node.attr.indicateKeepLeft = false;
			node.attr.indicateKeepRight = false;
			node.attr.slipLane = false;
			...
			link.attr.width = -1;
			link.attr.narrowRoad = false;
			link.attr.gpsBothWays = false;
			link.attr.blockIfNoLanes = false;
			link.attr.shortcut = false;
			link.attr.dontUseForNavigation = false;
