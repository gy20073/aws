...
void Scenario::parseScenarioConfig(const Value& sc, bool setDefaults) {
	...
	if (drivingMode.IsArray()) {
		...
		else if (setDefaults) _setSpeed = 25.0; // 1.0*(rand() % 20);
	}
	else if (setDefaults) {
		_drivingMode = -1;
	}
...
void Scenario::parseDatasetConfig(const Value& dc, bool setDefaults) {
	...
	if (dc["reward"].IsArray()) {
		if (dc["reward"][0].IsFloat() && dc["reward"][1].IsFloat()) {
			rewarder = new GeneralRewarder(lanerewarder, dc["reward"][0].GetFloat(), dc["reward"][1].GetFloat());
	...
	if (!dc["collision"].IsNull()) collision = dc["collision"].GetBool();
	else if (setDefaults) collision = _COLLISION_;
	if (!dc["roadinfo"].IsNull()) roadinfo = dc["roadinfo"].GetBool();
	else if (setDefaults) roadinfo = _ROADINFO_;
	...
	if (roadinfo) d.AddMember("roadinfo", a, allocator);
	...
	d.AddMember("cam_far_clip", 0.0, allocator);
	d.AddMember("cam_field_of_view", 0.0, allocator);

	// timestamp
	d.AddMember("timestamp", 0.0, allocator);
...
void Scenario::buildScenario() {
	...
	VEHICLE::SET_VEHICLE_COLOURS(vehicle, 27, 28);
	...
	birdcamera = CAM::CREATE_CAM("DEFAULT_SCRIPTED_CAMERA", TRUE);
    ...
    CAM::SET_CAM_COORD(birdcamera, pos.x, pos.y, pos.z + 30.0);
	CAM::SET_CAM_FOV(birdcamera, 60);
	CAM::SET_CAM_ROT(birdcamera, rotation.x-90, rotation.y, rotation.z+90, 1);
	CAM::SET_CAM_FOV(camera, 60);
	CAM::SET_CAM_ACTIVE(camera, TRUE);
	//CAM::SET_CAM_ACTIVE(birdcamera, TRUE);
	...
	if (_drivingMode >= 0) {
		if (!direction) AI::TASK_VEHICLE_DRIVE_WANDER(ped, vehicle, _setSpeed, _drivingMode);
		else AI::TASK_VEHICLE_DRIVE_TO_COORD(ped, vehicle, dir.x, dir.y, dir.z, _setSpeed, 1.0, vehicleHash, _drivingMode, 1.0, true);
		//else AI::TASK_VEHICLE_DRIVE_TO_COORD_LONGRANGE(ped, vehicle, dir.x, dir.y, dir.z, _setSpeed, _drivingMode, 10.0);
	}
	UI::REFRESH_WAYPOINT();
	if (direction) UI::SET_NEW_WAYPOINT(dir.x, dir.y);
...
void Scenario::start(const Value& sc, const Value& dc) {
	...
	//STREAMING::SET_VEHICLE_POPULATION_BUDGET(20);
...
void Scenario::run() {
	if (running) {
		...
		//VEHICLE::SET_VEHICLE_DENSITY_MULTIPLIER_THIS_FRAME(15.0f);
		...
		if (delay > 15) {
			...
			PED::SET_DRIVER_AGGRESSIVENESS(ped, 10.0);
			PED::SET_DRIVER_ABILITY(ped, 100.0);
			Any peds[arrSize];
			peds[0] = numElements;
			int count = PED::GET_PED_NEARBY_PEDS(PLAYER::PLAYER_PED_ID(), (int *)peds, -1);
			for (int i = 0; i < count; ++i)
			{
				int offsettedID = i * 2 + 2;
				if (ENTITY::DOES_ENTITY_EXIST(peds[offsettedID]))
				{
					AI::SET_DRIVE_TASK_DRIVING_STYLE(peds[offsettedID], 786603);
					/*float prob = static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
					if (prob > 0.2) {
						AI::SET_DRIVE_TASK_DRIVING_STYLE(peds[offsettedID], 786603);
					}
					else {
						AI::SET_DRIVE_TASK_DRIVING_STYLE(peds[offsettedID], 134);
					}*/
				}
			}
		}
...
void Scenario::stop() {
	...
	AI::CLEAR_PED_TASKS(ped);
	setCommands(0.0, 0.0, 0.0, -1.0);
}

void Scenario::setCommands(float throttle, float brake, float steering, int manual) {
	if (manual == 1) {
		AI::CLEAR_PED_TASKS(ped);
	}
	if (manual == 0) {
		if (_drivingMode >= 0) {
			if (!direction) AI::TASK_VEHICLE_DRIVE_WANDER(ped, vehicle, _setSpeed, _drivingMode);
			else AI::TASK_VEHICLE_DRIVE_TO_COORD(ped, vehicle, dir.x, dir.y, dir.z, _setSpeed, 1.0, GAMEPLAY::GET_HASH_KEY((char*)_vehicle), _drivingMode, 1.0, true);
		}
	}
...
void Scenario::setVehiclesList() {
	...
	for (int i = 0; i < count; i++) {
		...
		if (ENTITY::IS_ENTITY_ON_SCREEN(vehicles_list[i])) {
			...
			if(true){
				if (true){
					bool visible = ENTITY::HAS_ENTITY_CLEAR_LOS_TO_ENTITY(vehicle, vehicles_list[i], 17);
					...
					_vehicle.AddMember("arrID", (UINT32)vehicles_list[i] + (1 << 14), allocator);
...
void Scenario::setPedsList(){
	...
	for (int i = 0; i < count; i++) {
		...
		if (ENTITY::IS_ENTITY_ON_SCREEN(peds_list[i])) {
			...
			if (true){
				if (true){
					bool visible = ENTITY::HAS_ENTITY_CLEAR_LOS_TO_ENTITY(ped, peds_list[i], 17);
					...
					_ped.AddMember("arrID", (UINT32)peds_list[i] + (1 << 14), allocator);
...
void Scenario::setSteering(){
	d["steering"] = - getFloatValue(vehicle, 0x904) / 0.6981317008; // 0x8F4 
...
void Scenario::setDirection() {
	...
	distance_remaining = PATHFIND::CALCULATE_TRAVEL_DISTANCE_BETWEEN_POINTS(pos.x, pos.y, pos.z, dir.x, dir.y, dir.z);
	Value _direction(kArrayType);
	_direction.PushBack(direction, allocator).PushBack(distance, allocator).PushBack(distance_remaining, allocator);
...
void Scenario::setCamIntrinsic() {
	...
	// far clip
	float far_clip = CAM::GET_CAM_FAR_CLIP(camera);
	d["cam_far_clip"] = far_clip;
	// field of view
	d["cam_field_of_view"] = CAM::GET_CAM_FOV(camera);
	
}

void Scenario::setTimestamp() {
	auto now = std::chrono::system_clock::now();
	d["timestamp"] = (uint64_t)(std::chrono::duration<double>(now.time_since_epoch()).count() * 1000);
}

void Scenario::setroadinfo()
{
	if(!reward) lanerewarder->computeReward(vehicle);
	Document::AllocatorType& allocator = d.GetAllocator();
	Value roadinfo(kArrayType);
	//roadinfo.PushBack((*(lanerewarder)).isOnroad, allocator);
	//roadinfo.PushBack((*(lanerewarder)).cannotaccessroadinfo, allocator);
	Vector3 forwardVector = ENTITY::GET_ENTITY_FORWARD_VECTOR(vehicle);;
	roadinfo.PushBack(forwardVector.x, allocator).PushBack(forwardVector.y, allocator).PushBack(forwardVector.z, allocator);
	Vector3 velocity = ENTITY::GET_ENTITY_VELOCITY(vehicle);
	roadinfo.PushBack(velocity.x, allocator).PushBack(velocity.y, allocator).PushBack(velocity.z, allocator);
	/*if ((*(lanerewarder)).isOnroad && (!(*(lanerewarder)).cannotaccessroadinfo)) {
		Vector3 roadVector = (*(lanerewarder)).road_.direction;
		roadinfo.PushBack(roadVector.x, allocator).PushBack(roadVector.y, allocator).PushBack(roadVector.z, allocator);
		roadinfo.PushBack((*(lanerewarder)).pointPair.at(0).coord.x, allocator).PushBack((*(lanerewarder)).pointPair.at(0).coord.y, allocator)
			.PushBack((*(lanerewarder)).pointPair.at(0).coord.z, allocator);
		roadinfo.PushBack((*(lanerewarder)).pointPair.at(1).coord.x, allocator).PushBack((*(lanerewarder)).pointPair.at(1).coord.y, allocator)
			.PushBack((*(lanerewarder)).pointPair.at(1).coord.z, allocator);
		bool laneIn = (*(lanerewarder)).pointPair.at(0).laneIn || (*(lanerewarder)).pointPair.at(1).laneIn;
		roadinfo.PushBack(laneIn, allocator);
		roadinfo.PushBack((*(lanerewarder)).road.attr.cantGoLeft, allocator)
			.PushBack((*(lanerewarder)).road.attr.cantGoRight, allocator)
			.PushBack((*(lanerewarder)).road.attr.density, allocator)
			.PushBack((*(lanerewarder)).road.attr.disabled, allocator)
			.PushBack((*(lanerewarder)).road.attr.highway, allocator)
			.PushBack((*(lanerewarder)).road.attr.indicateKeepLeft, allocator)
			.PushBack((*(lanerewarder)).road.attr.indicateKeepRight, allocator)
			.PushBack((*(lanerewarder)).road.attr.leftTurnsOnly, allocator)
			.PushBack((*(lanerewarder)).road.attr.noBigVehicles, allocator)
			.PushBack((*(lanerewarder)).road.attr.noGPS, allocator)
			.PushBack((*(lanerewarder)).road.attr.offRoad, allocator)
			.PushBack((*(lanerewarder)).road.attr.slipLane, allocator)
			.PushBack((*(lanerewarder)).road.attr.special, allocator)
			.PushBack((*(lanerewarder)).road.attr.speed, allocator)
			.PushBack((*(lanerewarder)).road.attr.tunnel, allocator)
			.PushBack((*(lanerewarder)).road.attr.water, allocator);
		roadinfo.PushBack((*(lanerewarder)).road_.attr.blockIfNoLanes, allocator)
			.PushBack((*(lanerewarder)).road_.attr.gpsBothWays, allocator)
			.PushBack((*(lanerewarder)).road_.attr.narrowRoad, allocator)
			.PushBack((*(lanerewarder)).road_.attr.shortcut, allocator)
			.PushBack((*(lanerewarder)).road_.attr.width, allocator);			
	}*/
	d["roadinfo"] = roadinfo;
}

void Scenario::initRewarder() {
	lanerewarder = new LaneRewarder((char*)(GetCurrentModulePath() + "paths.xml").c_str());
}
