...
class Depth:
    def __init__(self,
        ...
        params = monodepth_parameters(
            ...
            height=self.height,
            width=self.width,
            batch_size=2, # since we are doing averaging by flipping
            num_threads=1,
            num_epochs=1,
            do_stereo=False,
            wrap_mode="border",
        ...
        left = tf.placeholder(tf.float32, [2, self.height, self.width, 3])
    ...
    def compute_disparity_average(self, image):
        # assume a RGB input image
        input_image = scipy.misc.imresize(image, [self.height, self.width], interp='bilinear')
        input_image = input_image.astype(np.float32) / 255
        input_images = np.stack((input_image, np.fliplr(input_image)), 0)

        disp = self.sess.run(self.model.disp_left_est[0],
                             feed_dict={self.left: input_images})
        disp_pp = self.post_process_disparity(disp.squeeze()).astype(np.float32)
        # convert w*h image to w*h*1
        disp_pp = np.expand_dims(disp_pp, axis=2)
        return disp_pp

    def visualize_disparity_average(self, pred):
        pred = pred.squeeze()
